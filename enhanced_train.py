...
from dataset import (
    ClaimsMaskedDataset,
    StreamingClaimsMaskedDataset,
    SequentialStreamingClaimsMaskedDataset,
)
import time
...
if TORCH_VERSION >= "2.0.0":
    ...
    logger.warning(
        f"PyTorch {TORCH_VERSION} does not support torch.compile (requires 2.0+)"
    )
...
def load_data(
    ...
    try:
        train_dataset = SequentialStreamingClaimsMaskedDataset(
            ...
            token_budgets=[60, 11, 15, 2, 5, 5, 26, 7, 1, 3],
        )
    ...
    if hasattr(train_dataset, "total_examples") and hasattr(
        train_dataset, "line_count"
    ):
        if train_dataset.line_count > 0:
            avg_examples_per_line = (
                train_dataset.total_examples / train_dataset.line_count
            )

    total_examples = getattr(
        train_dataset, "total_examples", 1000
    )  # Fallback to 1000 if not available
    ...
    def worker_init_fn(worker_id):
        ...
        )  # Use default handler for clean exit
    ...
    logger.info(
        f"  Total training capacity: {estimated_train_batches * batch_size:,} examples"
    )
...
def create_dataloaders(dataset, train_sampler, val_sampler, batch_size, num_workers):
    ...
    # Create training dataloader
    train_dataloader = DataLoader(dataset, sampler=train_sampler, **dataloader_args)
    ...
    val_dataloader = DataLoader(dataset, sampler=val_sampler, **dataloader_args)
...
def validate(model, dataloader, device):
    ...
    val_batches = min(
        max(
            10, int(len(dataloader) * val_percentage)
        ),  # At least 10 batches but at most 10% of validation set
        1000,  # Hard cap at 1000 batches to keep validation time reasonable
    )

    logger.info(
        f"Running validation on {val_batches} batches (out of {len(dataloader):,} total validation batches)"
    )
    ...
    try:
        ...
        with torch.no_grad():
            for step in val_progress_bar:
                ...
                try:
                    ...
                    logger.warning(
                        f"Validation dataloader exhausted early at step {step}/{val_batches}"
                    )
                    if step == 0:
                ...
                if (
                    not args.disable_wandb and step % 5 == 0
                ):  # Only log every 5 steps to avoid too many points
                    wandb.log(
                        {
        ...
        logger.info(
            f"Validation completed: Loss={avg_loss:.4f}, Perplexity={np.exp(avg_loss):.2f}, "
...
def save_checkpoint(
    model, optimizer, scheduler, epoch, args, val_loss, best=False, step=None
):
    ...
    if step is not None:
        ...
        checkpoint_path = os.path.join(
            checkpoint_dir, f"checkpoint_epoch_{epoch}_step_{step}.pt"
        )
    elif best:
    ...
    if step is not None:
        tokenizer_dir = os.path.join(
            checkpoint_dir, f"tokenizer_epoch_{epoch}_step_{step}"
        )
...
def sample_masked_token(input, logits, tokenizer, candidates=[]):
    ...
    for i in range(input.shape[0]):
        ...
        if candidates:
            encoded_candidates = [
                tokenizer.encode(cand, add_special_tokens=False) for cand in candidates
            ]
            scores = []
...
def run_example_inference(model, tokenizer, test_cases, device):
    ...
    with torch.no_grad():
        for case in test_cases:
            ...
            inputs = tokenizer(
                query, return_tensors="pt", padding="max_length", truncation=True
            ).to(device)
            logits = model(**inputs).logits

            if candidates:
                predictions, candidate_scores = sample_masked_token(
                    inputs.input_ids, logits, tokenizer, candidates
                )
                ...
                if len(candidate_scores) == 1:
                    ...
                    result = {
                        ...
                        "predictions": predictions,
                        "scores": [
                            [float(s) for s in score_set]
                            for score_set in candidate_scores
                        ],
                    }
            else:
                predictions = sample_masked_token(inputs.input_ids, logits, tokenizer)
                result = {"query": case["query"], "predictions": predictions}
            ...
            if candidates:
                ...
                if len(candidate_scores) == 1:
                    for i, (candidate, score) in enumerate(
                        zip(candidates, candidate_scores[0])
                    ):
                        logger.info(f"Candidate {i+1}: {candidate}, Score: {score:.4f}")
                else:
                    logger.info(
                        "Multiple mask positions detected, showing candidate scores:"
                    )
                    for i, score_set in enumerate(candidate_scores):
                        logger.info(
                            f"  Position {i+1}: {[f'{s:.4f}' for s in score_set]}"
                        )
...
def get_cosine_with_hard_restarts_schedule_with_min_lr(
    optimizer, num_warmup_steps, num_training_steps, num_cycles=1, min_lr_ratio=0.0
):
    """
...
def main(args):
    ...
    try:
        ...
        logger.info(
            f"TensorBoard logs will be saved to {os.path.join(args.output_dir, 'tensorboard')}"
        )
    except Exception as e:
    ...
    special_tokens = {"additional_special_tokens": ["<tab>", "<colpad>"]}
    tokenizer.add_special_tokens(special_tokens)
    logger.info(
        f"Added <tab> special token (ID: {tokenizer.convert_tokens_to_ids('<tab>')}"
    )
    logger.info(
        f"Added <colpad> special token (ID: {tokenizer.convert_tokens_to_ids('<colpad>')})"
    )
    ...
    if args.compile and TORCH_COMPILE_SUPPORTED:
        ...
        logger.warning(
            "Model compilation requested but not supported by PyTorch version. Skipping."
        )
        ...
        if wandb_enabled:
            ...
            try:
                lr_table = wandb.Table(
                    data=lr_schedule, columns=["step", "learning_rate"]
                )
                wandb.log(
                    {
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for step in progress_bar:
            ...
            try:
                ...
                logger.warning(
                    f"Dataloader exhausted at step {step}/{total_batches} (this shouldn't happen)"
                )
                break
            ...
            if global_step % 1000 == 0:
                ...
                try:
                    if val_dataloader is not None:
                        ...
                        logger.info(
                            f"Validation loss at step {global_step}: {step_val_loss:.4f}"
                        )
                        ...
                        if wandb_enabled:
                                ...
                                wandb.log(
                                    {
                                        ...
                                        "step_validation/perplexity": np.exp(
                                            step_val_loss
                                        ),
                            ...
                            if step_val_loss < best_val_loss:
                                logger.info(
                                    f"Validation loss improved from {best_val_loss:.4f} to {step_val_loss:.4f}"
                                )
                        ...
                        logger.info(
                            "Skipping validation step as validation is disabled"
                        )
            ...
            if tensorboard_enabled:
                try:
                    ...
                    writer.add_scalar(
                        "train/line_position",
                        train_dataloader.dataset.current_line,
                        global_step,
                    )
                except Exception as e:
                    ...
                    wandb.log(
                        {
                            ...
                            "train/epoch": epoch
                            + (step / total_batches),  # Fractional epoch
            ...
            if args.max_steps_per_epoch > 0 and step >= args.max_steps_per_epoch:
                logger.info(
                    f"Reached max steps per epoch ({args.max_steps_per_epoch}), ending epoch early"
                )
                break
        ...
        try:
            if val_dataloader is not None:
                ...
                # Log validation loss
                logger.info(
                    f"Validation loss: {val_loss:.4f}, Perplexity: {np.exp(val_loss):.2f}"
                )
                ...
                if val_loss < best_val_loss:
                    ...
                    save_checkpoint(
                        model, optimizer, scheduler, epoch, args, val_loss, best=True
                    )
            else:
        ...
        if tensorboard_enabled:
            try:
                ...
                writer.add_scalar(
                    "epoch/total_tokens_processed", total_tokens_processed, epoch
                )
        ...
        if args.early_stopping > 0 and no_improvement_count >= args.early_stopping:
            logger.info(
                f"Early stopping triggered after {no_improvement_count} epochs without improvement"
            )
            break
    ...
    logger.info(f"Total tokens processed: {total_tokens_processed:,}")
    logger.info(
        f"Average processing speed: {total_tokens_processed / total_training_time:.2f} tokens/second"
    )
        ...
        try:
            ...
            try:
                loss_table = wandb.Table(
                    data=loss_plot_data, columns=["epoch", "train_loss"]
                )
                val_loss_table = wandb.Table(
                    data=val_loss_plot_data, columns=["epoch", "val_loss"]
                )
                ...
                # Perplexity plot
                train_ppl_table = wandb.Table(
                    data=train_ppl_data, columns=["epoch", "train_perplexity"]
                )
                val_ppl_table = wandb.Table(
                    data=val_ppl_data, columns=["epoch", "val_perplexity"]
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train a RoBERTa model for masked language modeling on claims data"
    )
    ...
    parser.add_argument(
        "--force_reload_data", action="store_true", help="Force reprocessing the data"
    )
    parser.add_argument(
        "--sample_percentage",
        type=float,
        default=100,
        help="Percentage of the dataset to use (1-100)",
    )
    parser.add_argument(
        "--max_rows",
        type=int,
    ...
    parser.add_argument(
        ...
        type=float,
        default=3e-5,
        help="Final learning rate (min value for cosine scheduler)",
    )
    parser.add_argument(
        "--learning_rate",
        type=float,
        default=5e-5,
        help="Learning rate (legacy parameter, use initial_lr instead)",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="Proportion of training to perform warmup",
    )
    parser.add_argument(
        "--early_stopping",
        type=int,
    ...
    parser.add_argument(
        "--inference_interval", type=int, default=5, help="Run inference every N epochs"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="auto",
        choices=["auto", "reduce-overhead", "max-autotune"],
    ...
    parser.add_argument(
        "--disable_inference",
        action="store_true",
        help="Disable example inference testing",
    )
